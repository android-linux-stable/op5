...
struct pn544_dev    {
	...
	/* used to make p61_current_state flag secure */
...
static unsigned int nfc_kernel_log;
...
static ssize_t pn544_dev_write(struct file *filp,
 ...
 */
static int nqx_ese_pwr_contrl(struct pn544_dev *pn544_dev,
					  ...
					  unsigned long arg)
{
	...
	case P61_SET_WIRED_ACCESS: {
		...
		if (arg == 1) {
			...
			pr_info("%s:P61_SET_WIRED_ACCESS-disable\n", __func__);
			if (current_state & P61_STATE_WIRED) {
				...
				struct pn544_i2c_platform_data *data)
{
	...
	r = of_property_read_string(np, "qcom,clk-src",
					&data->clk_src_name);


	pr_info("%s: %d, %d, %d, %d ,%d, %d %d\n", __func__,
		data->irq_gpio, data->ven_gpio, data->firm_gpio,
		data->ese_pwr_gpio, data->clk_gpio, data->wake_up_gpio,
		errorno);
					   ...
					   const struct i2c_device_id *id)
{
	...
	/* NFC don't contrl ldo 9 .ldo 9 contrled by modem,
	 * this will cause can't recognise sim card
	 */
	/* pn544_dev->p544_regulator = regulator_get( &client->dev,
	 * "nfc_voltage");
	 ...
	 * }
	 * ret = regulator_set_voltage(pn544_dev->p544_regulator,
	 * 1800000, 1800000);
	 * if (ret != 0)
	 * {
	...
	pn544_dev->p544_regulator_s4 = regulator_get(&client->dev,
								   "nfc_voltage_s4");
	...
	ret = regulator_set_voltage(pn544_dev->p544_regulator_s4, 1800000,
								1800000);
	if (ret != 0) {
	...
	if (ret < 0) {
		pr_err("%s :unable to set irq_gpio as input\n", __func__);
		goto err_gpio_set;
	}
	ret = gpio_direction_output(pn544_dev->ven_gpio, 0);
	if (ret < 0) {
		pr_err("%s : unable to set ven_gpio as output\n", __func__);
		goto err_gpio_set;
	}
	ret = gpio_direction_output(pn544_dev->ese_pwr_gpio, 0);
	if (ret < 0) {
		pr_err("%s : unable to set ese_pwr gpio as output\n", __func__);
		goto err_gpio_set;
	}

	ret = gpio_direction_output(pn544_dev->firm_gpio, 0);
	if (ret < 0) {
		pr_err("%s : unable to set firm_gpio as output\n", __func__);
		goto err_gpio_set;
	}
	ret = gpio_direction_input(pn544_dev->clk_gpio);
	if (ret < 0) {
		pr_err("%s : unable to set clk_gpio as input\n", __func__);
		goto err_gpio_set;
	}
	ret = gpio_direction_output(pn544_dev->wake_up_gpio, 0);
	if (ret < 0) {
		pr_err("%s : unable to set wake_up_gpio as output\n", __func__);
		goto err_gpio_set;
	}
	...
	wakeup_source_init(&pn544_dev->pn544_ws, "pn544_ws");
	/* End, NFC CLK_REQ*/
...
static int pn544_remove(struct i2c_client *client)
{
	...
	misc_deregister(&pn544_dev->pn544_device);
	wakeup_source_trash(&pn544_dev->pn544_ws);
	mutex_destroy(&pn544_dev->read_mutex);
	mutex_destroy(&pn544_dev->p61_state_mutex);
...
#if DRAGON_NFC
static const struct of_device_id pn544_i2c_dt_match[] = {
	{
